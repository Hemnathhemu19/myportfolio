...
const toolsLeft = [
    { id: 1, name: 'Slack', logo: 'https://cdn.simpleicons.org/slack', category: 'Collaboration' },
    { id: 2, name: 'Jitter', icon: <Move3d className="w-8 h-8 text-orange-400" />, category: 'Motion Design' }, // Fallback for Jitter
    { id: 3, name: 'Figma', logo: 'https://cdn.simpleicons.org/figma', category: 'UI/UX Design' },
    { id: 4, name: 'Webflow', logo: 'https://cdn.simpleicons.org/webflow/4353FF', category: 'Development' },
];

const toolsRight = [
    { id: 5, name: 'ChatGPT', logo: 'https://cdn.simpleicons.org/openai', category: 'AI Research' },
    { id: 6, name: 'Notion', logo: 'https://cdn.simpleicons.org/notion', category: 'Documentation' },
    { id: 7, name: 'React', logo: 'https://cdn.simpleicons.org/react/61DAFB', category: 'Frontend' },
    { id: 8, name: 'Google Meet', logo: 'https://cdn.simpleicons.org/googlemeet', category: 'Communication' },
];
...
export default function Tools() {
    ...
    return (
        <section ref={containerRef} className="relative w-full min-h-screen py-24 bg-black overflow-hidden flex flex-col items-center justify-center">
            ...
            <div className="relative w-full max-w-7xl flex flex-col md:flex-row items-center justify-between px-4 md:px-12">
                ...
                <div className="absolute inset-0 hidden md:block w-full h-[800px] pointer-events-none top-1/2 -translate-y-1/2">
                    <svg viewBox="0 0 1200 800" className="w-full h-full opacity-30">
                        ...
                        {toolsLeft.map((_, i) => (
                            <motion.path
                                key={`left-path-${i}`}
                                d={`M 200 ${120 + i * 160} C 400 ${120 + i * 160}, 400 400, 600 400`}
                                stroke="white"
                                strokeWidth="1.5"
                                fill="none"
                                strokeDasharray="6 6"
                        ...
                        {toolsRight.map((_, i) => (
                            <motion.path
                                key={`right-path-${i}`}
                                d={`M 1000 ${120 + i * 160} C 800 ${120 + i * 160}, 800 400, 600 400`}
                                stroke="white"
                                strokeWidth="1.5"
                                fill="none"
                                strokeDasharray="6 6"
                ...
                {/* Right Column */}
                <div className="flex flex-col gap-16 z-10 w-full md:w-1/3 items-center md:items-end pr-0 md:pr-10" dir="rtl">
                  ...
                  */}
                    {toolsRight.map((tool, i) => (
                        <motion.div
                            ...
                            animate={isInView ? { opacity: 1, x: 0 } : {}}
                            transition={{ duration: 0.5, delay: i * 0.1 }}
                            className="flex items-center gap-6 group flex-row-reverse" // Reverse flex for Right alignment
                        >
                            <div className="w-20 h-20 rounded-2xl bg-white border border-white/10 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                                {tool.logo ? (
                                    <img src={tool.logo} alt={tool.name} className="w-10 h-10 object-contain" />
